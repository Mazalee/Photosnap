export const imageData = [
  {
    id: 1,
    src: "/home/mobile/create-and-share.jpg",
    alt: "beautiful-stories",
    header: "create and share your photo stories.",
    paragraph:
      "Photosnap is a platform for photographers and visual storytellers. We make it easy to share photos, tell stories and connect with others.",
    button: "get an invite",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    bgcolor: "#000000",
    color: "#ffffff",
  },
  {
    id: 2,
    src: "/home/mobile/beautiful-stories.jpg",
    alt: "beautiful-stories",
    header: "beautiful stories every time",
    paragraph:
      "We provide design templates to ensure your stories look terrific. Easily add photos, text, embed maps and media from other networks. Then share your story with everyone.",
    button: "view the stories",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#000">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    bgcolor: "#ffffff",
    color: "#000000",
  },
  {
    id: 3,
    src: "/home/mobile/designed-for-everyone.jpg",
    alt: "beautiful-stories",
    header: "designed for everyone",
    paragraph:
      "Photosnap can help you create stories that resonate with your audience.  Our tool is designed for photographers of all levels, brands, businesses you name it.",
    button: "view the stories",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#000">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    bgcolor: "#ffffff",
    color: "#000000",
  },
];

export const dataImages = [
  {
    id: 1,
    src: "/stories/mobile/mountains.jpg",
    alt: "mountains",
    header: "the mountains",
    paragraph: "by John Appleseed",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 2,
    src: "/stories/mobile/cityscapes.jpg",
    alt: "city scapes",
    header: "sunset cityscapes",
    paragraph: "by Benjamin Cruz",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 3,
    src: "/stories/mobile/18-days-voyage.jpg",
    alt: "voyage",
    header: "18 days voyage",
    paragraph: "by Alexei Borodin",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 4,
    src: "/stories/mobile/architecturals.jpg",
    alt: "architecturals",
    header: "architecturals",
    paragraph: "by Samantha Brooke",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
];

export const featuresInfo = [
  {
    id: 1,
    src: "/features/mobile/hero.jpg",
    alt: "hero",
    header: "features",
    bgcolor: "#000000",
    color: "#ffffff",
    paragraph:
      "We make sure all of our features are designed to be loved by every aspiring and even professional photograpers who wanted to share their stories.",
  },
];

export const featuresImageBtm = [
  {
    id: 1,
    src: "/shared/mobile/bg-beta.jpg",
    alt: "beta",
    header: "we're in beta. get your invite today!",
    button: "get an invite",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
  },
];

export const featuresData = [
  {
    id: 1,
    src: "/features/desktop/responsive.svg",
    alt: "responsive",
    header: "100% responsive",
    paragraph:
      "No matter which the device you're on, our site is fully responsive and stories look beautiful on any screen.",
  },
  {
    id: 2,
    src: "/features/desktop/no-limit.svg",
    alt: "no limit",
    header: "no photo upload limit",
    paragraph:
      "Our tool has no limits on uploads or bandwidth. Freely upload in bulk and share all of your stories in one go.",
  },
  {
    id: 3,
    src: "/features/desktop/embed.svg",
    alt: "embed",
    header: "available to embed",
    paragraph:
      "Embed Tweets, Facebook posts, Instagram media, Vimeo or YouTube videos, Google Maps, and more. ",
  },
  {
    id: 4,
    src: "/features/desktop/custom-domain.svg",
    alt: "custom",
    header: "custom domain",
    paragraph:
      "With Photosnap subscriptions you can host your stories on your own domain. You can also remove our branding!. ",
  },
  {
    id: 5,
    src: "/features/desktop/boost-exposure.svg",
    alt: "boost your exposure",
    header: "boost your exposure",
    paragraph:
      "Users that viewed your story or gallery can easily get notifed of new and featured stories with our built in mailing list. ",
  },
  {
    id: 6,
    src: "/features/desktop/drag-drop.svg",
    alt: "drag drop",
    header: "drag & drop image",
    paragraph:
      "Easily drag and drop your image and get beautiful shots everytime. No over the top tooling to add friction to creating stories. ",
  },
];

export const socialIcons = [
  {
    id: 1,
    src: "/shared/desktop/facebook.svg",
    alt: "facebook",
  },
  {
    id: 2,
    src: "/shared/desktop/youtube.svg",
    alt: "youtube",
  },
  {
    id: 3,
    src: "/shared/desktop/twitter.svg",
    alt: "twitter",
  },
  {
    id: 4,
    src: "/shared/desktop/pinterest.svg",
    alt: "pinterest",
  },
  {
    id: 5,
    src: "/shared/desktop/instagram.svg",
    alt: "instagram",
  },
];

export const storiesImage = [
  {
    id: 1,
    src: "/stories/mobile/moon-of-appalacia.jpg",
    alt: "appalacia",
    detail: "last month's featured story",
    header: "hazy full moon of appalachia",
    author: "March 2nd 2020 by John Appleseed",
    paragraph:
      "The dissected plateau area, while not actually made up of geological mountains, is popularly called mountains, especially in eastern Kentucky and West Virginia, and while the ridges are not high, the terrain is extremely rugged.",
    button: "read the story",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    bgcolor: "#000000",
    color: "#ffffff",
  },
];

export const storiesData = [
  {
    id: 1,
    src: "/stories/mobile/mountains.jpg",
    alt: "mountains",
    date: "April 16th 2020",
    header: "the mountains",
    paragraph: "by John Appleseed",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 2,
    src: "/stories/mobile/cityscapes.jpg",
    alt: "city scapes",
    date: "April 14th 2020",
    header: "sunset cityscapes",
    paragraph: "by Benjamin Cruz",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 3,
    src: "/stories/mobile/18-days-voyage.jpg",
    alt: "voyage",
    date: "April 11th 2020",
    header: "18 days voyage",
    paragraph: "by Alexei Borodin",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 4,
    src: "/stories/mobile/architecturals.jpg",
    alt: "architecturals",
    date: "April 9th 2020",
    header: "architecturals",
    paragraph: "by Samantha Brooke",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 5,
    src: "/stories/mobile/world-tour.jpg",
    alt: "world tour",
    date: "April 7th 2020",
    header: "world tour 2019",
    paragraph: "by Timothy Wagner",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 6,
    src: "/stories/mobile/unforeseen-corners.jpg",
    alt: "unforeseen",
    date: "April 3rd 2020",
    header: "unforeseen corners",
    paragraph: "by William Malcolm",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 7,
    src: "/stories/mobile/king-on-africa.jpg",
    alt: "king on africa",
    date: "March 29th 2020",
    header: "king on africa: part II",
    paragraph: "by Tim Hillenburg",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 8,
    src: "/stories/mobile/trip-to-nowhere.jpg",
    alt: "trip to nowhere",
    date: "March 21st 2020",
    header: "the trip to nowhere",
    paragraph: "by Felicia Rourke",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 9,
    src: "/stories/mobile/rage-of-the-sea.jpg",
    alt: "rage of the sea",
    date: "March 19th 2020",
    header: "rage of the sea",
    paragraph: "by Mohammed Abdul",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 10,
    src: "/stories/mobile/running-free.jpg",
    alt: "running free",
    date: "March 16th 2020",
    header: "running free",
    paragraph: "by Michelle",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 11,
    src: "/stories/mobile/behind-the-waves.jpg",
    alt: "behind the waves",
    date: "March 11th 2020",
    header: "behind the waves",
    paragraph: "by Lamarr Wilson",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 12,
    src: "/stories/mobile/calm-waters.jpg",
    alt: "calm waters",
    date: "March 9th 2020",
    header: "calm waters",
    paragraph: "by Samantha Brooke",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 13,
    src: "/stories/mobile/milky-way.jpg",
    alt: "milky way",
    date: "March 5th 2020",
    header: "the milky way",
    paragraph: "by Benjamin Cruz",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 14,
    src: "/stories/mobile/dark-forest.jpg",
    alt: "dark forest",
    date: "March 4th 2020",
    header: "night at the dark forest",
    paragraph: "by Mohammed Abdul",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 15,
    src: "/stories/mobile/somwarpet.jpg",
    alt: "somwarpet",
    date: "March 1st 2020",
    header: "somwarpet's beauty",
    paragraph: "by Michelle",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
  {
    id: 16,
    src: "/stories/mobile/land-of-dreams.jpg",
    alt: "land of dreams",
    date: "February 25th 2020",
    header: "land of dreams",
    paragraph: "by William Malcolm",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="43" height="14">
        <g fill="none" fill-rule="evenodd" stroke="#fff">
          <path d="M0 7h41.864M35.428 1l6 6-6 6" />
        </g>
      </svg>
    ),
    button: "read story",
  },
];

export const featuresImage = [
  {
    id: 1,
    src: "/features/desktop/responsive.svg",
    alt: "responsive",
    header: "100% responsive",
    paragraph:
      "No matter which the device you're on, our site is fully responsive and stories look beautiful on any screen.",
  },
  {
    id: 2,
    src: "/features/desktop/no-limit.svg",
    alt: "no limit",
    header: "no photo upload limit",
    paragraph:
      "Our tool has no limits on uploads or bandwidth. Freely upload in bulk and share all of your stories in one go.",
  },
  {
    id: 3,
    src: "/features/desktop/embed.svg",
    alt: "embed",
    header: "available to embed",
    paragraph:
      "Embed Tweets, Facebook posts, Instagram media, Vimeo or YouTube videos, Google Maps, and more. ",
  },
];

export const pricingInfo = [
  {
    id: 1,
    src: "/pricing/mobile/hero.jpg",
    alt: "hero",
    header: "pricing",
    bgcolor: "#000000",
    color: "#ffffff",
    paragraph:
      "Create a your stories, Photosnap is a platform for photographers and visual storytellers. It’s the simple way to create and share your photos.",
  },
];

export const pricingData = [
  {
    id: 1,
    header: "basic",
    paragraph:
      "Includes basic usage of our platform. Recommended for new and aspiring photographers.",
    price: "19.00",
    time: "per month",
    button: "pick plan",
    bgcolor: "#ffffff",
    color: "#000000",
    bgcolor_btn: "#000000",
    color_btn: "#ffffff",
  },
  {
    id: 2,
    header: "pro",
    paragraph:
      "More advanced features available. Recommended for photography veterans and professionals.",
    price: "39.00",
    time: "per month",
    button: "pick plan",
    bgcolor: "#000000",
    color: "#ffffff",
    bgcolor_btn: "#ffffff",
    color_btn: "#000000",
  },
  {
    id: 3,
    header: "business",
    paragraph:
      "Additional features available such as more detailed metrics. Recommended for business owners.",
    price: "99.00",
    time: "per month",
    button: "pick plan",
    bgcolor: "#ffffff",
    color: "#000000",
    bgcolor_btn: "#000000",
    color_btn: "#ffffff",
  },
];

export const planPackages = [
  {
    id: 1,
    header: "unlimited story posting",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BASIC", "PRO", "BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 2,
    header: "unlimited photo upload",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BASIC", "PRO", "BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 3,
    header: "embedding custom content",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["PRO", "BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 4,
    header: "customize metadata",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["PRO", "BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 5,
    header: "advanced metrics",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 6,
    header: "photo downloads",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 7,
    header: "search engine indexing",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
  {
    id: 8,
    header: "custom analytics",
    packages: ["BASIC", "PRO", "BUSINESS"],
    availablePackages: ["BUSINESS"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15">
        <path
          fill="none"
          stroke="#000"
          stroke-width="2"
          d="M1 8.124L5.623 13 17 1"
        />
      </svg>
    ),
  },
];
